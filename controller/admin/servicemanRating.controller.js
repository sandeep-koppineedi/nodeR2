...
exports.addService = async function (req, res) {
  try {
    ...
    if (saveServe) {
      ...
      res
        .status(400)
        .json({ message: "Serviceman rating could not be added!" });
    }
  } catch (err) {
...
// get all serviceman ratings
exports.getAllServicemanRatings = async function (req, res) {
  try {
    let condition = {};
    ...
    const ratings = await servicemanRatingModel
      .find(condition)
      .sort({ logCreatedDate: -1 });

    res.status(200).json({
      success: true,
      message: "Ratings have been retrieved successfully",
      ratings: ratings,
    });
  } catch (err) {
    res
