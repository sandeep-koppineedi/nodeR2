...
// exports.vendorRegisteration = async function (req, res) {
//   try {
...
// Login/Signin
exports.vendorLogin = async function (req, res) {
  // try {
  const user = await vendorModel.findOne({
...
exports.getVendorProfile = async function (req, res) {
  // try {
  const profileResult = await vendorModel.findOne(
    { _id: req.userId },
    { password: 0 }
  );
...
exports.editVendorProfile = async function (req, res) {
  try {
    const logDate = new Date().toISOString();
...
exports.changeVendorPassword = async function (req, res) {
  try {
    ...
    const userPass = await vendorModel.findOne(
      { _id: req.userId },
      { password: 1 }
    );
...
exports.generateOtp = async function (req, res) {
  ...
  if (user) {
    ...
    //   }
    if (otpSave) {
      const userdata = await vendorModel.findOne(
        { email: senderEmail },
        { _id: 1, email: 1 }
      );
...
exports.resetPassword = async function (req, res) {
  ...
  if (showEmail) {
    ...
    if (newpassword === confirmpassword) {
      ...
      const showUser = await vendorModel.updateOne(
...
exports.updateAdminRolesAndPermit = async function (req, res) {
  try {
    ...
    const employeeRole = await vendorModel.updateOne(
      { _id: req.body.id },
      {
        $set: {
